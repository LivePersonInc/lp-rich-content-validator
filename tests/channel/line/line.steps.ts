...
const feature = loadFeature('tests/channel/line/line.feature');
...
defineFeature(feature, (test) => {
  test('Validation works for all eligible test-JSONs (High Lvl)', ({ given, when, then }) => {
    const eligibleTestJsonContainers = testJsonContainers.filter(
      (testJsonContainer) => testJsonContainer().channelCompatibility[Channels.LINE] !== undefined,
    );
    const testExpectations: ITestExpectation[] = [];
    let validator: RichContentValidator;

    given('I have a Rich Content validator for Line', () => {
    ...
    then('The validator either validates or returns errors for them as expected on the Line channel', () => {
      testExpectations.forEach((testExpectation) => {
        ...
        const {
          testJson: {
            channelCompatibility: { [Channels.LINE]: lineCompatibility },
          },
        } = testExpectation;
        const result = testExpectation.validationResult;
        if (lineCompatibility) {
  ...
  test('Validation with exported Schemas works for all eligible test-JSONs (High Lvl)', ({ given, when, then }) => {
    ...
    given('I exported the Line Schemas from a Rich Content validator', () => {
      const validator = new RichContentValidator({ channel: Channels.LINE });
      const bodySchema = validator.exportSchema({ type: Types.BDY, channel: Channels.LINE });
      const metadataSchema = validator.exportSchema({ type: Types.MTD, channel: Channels.LINE });
      const quickrepliesSchema = validator.exportSchema({ type: Types.QUR, channel: Channels.LINE });
    ...
    when(
      ...
      () => {
        function validatingJson(json: object, isBody: boolean, validator: ValidateFunction): IValidationResult {
          try {
            if (isBody) {
              const sorter = new RichContentSorter({ channel: Channels.LINE });
