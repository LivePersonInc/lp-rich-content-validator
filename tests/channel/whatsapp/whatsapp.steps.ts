...
const feature = loadFeature('tests/channel/whatsapp/whatsapp.feature');
...
defineFeature(feature, (test) => {
  test('Validation works for all eligible test-JSONs (High Lvl)', ({ given, when, then }) => {
    const eligibleTestJsonContainers = testJsonContainers.filter(
      (testJsonContainer) => testJsonContainer().channelCompatibility[Channels.WA] !== undefined,
    );
    ...
    then('The validator either validates or returns errors for them as expected on the WhatsApp channel', () => {
      testExpectations.forEach((testExpectation) => {
        ...
        const {
          testJson: {
            channelCompatibility: { [Channels.WA]: whatsappCompatibility },
          },
        } = testExpectation;
        const result = testExpectation.validationResult;
        if (whatsappCompatibility) {
  ...
  test('Validation with exported Schemas works for all eligible test-JSONs (High Lvl)', ({ given, when, then }) => {
    ...
    given('I exported the WhatsApp Schemas from a Rich Content validator', () => {
      const validator = new RichContentValidator({ channel: Channels.WA });
      const bodySchema = validator.exportSchema({ type: Types.BDY, channel: Channels.WA });
      const metadataSchema = validator.exportSchema({ type: Types.MTD, channel: Channels.WA });
      const quickrepliesSchema = validator.exportSchema({ type: Types.QUR, channel: Channels.WA });
    ...
    when(
      ...
      () => {
        function validatingJson(json: object, isBody: boolean, validator: ValidateFunction): IValidationResult {
          try {
            if (isBody) {
              const sorter = new RichContentSorter({ channel: Channels.WA });
