...
const feature = loadFeature('tests/channel/rcsBusinessMessaging/rcsBusinessMessaging.feature');
...
defineFeature(feature, (test) => {
  test('Validation works for all eligible test-JSONs (High Lvl)', ({ given, when, then }) => {
    const eligibleTestJsonContainers = testJsonContainers.filter(
      (testJsonContainer) => testJsonContainer().channelCompatibility[Channels.RBM] !== undefined,
    );
    ...
    then('The validator either validates or returns errors for them as expected on the RBM channel', () => {
      testExpectations.forEach((testExpectation) => {
        ...
        const {
          testJson: {
            channelCompatibility: { [Channels.RBM]: rbmCompatibility },
          },
        } = testExpectation;
  ...
  test('Validation with exported Schemas works for all eligible test-JSONs (High Lvl)', ({ given, when, then }) => {
    ...
    given('I exported the RBM Schemas from a Rich Content validator', () => {
      const validator = new RichContentValidator({ channel: Channels.RBM });
      const bodySchema = validator.exportSchema({ type: Types.BDY, channel: Channels.RBM });
      const metadataSchema = validator.exportSchema({ type: Types.MTD, channel: Channels.RBM });
      const quickrepliesSchema = validator.exportSchema({ type: Types.QUR, channel: Channels.RBM });
    ...
    when(
      ...
      () => {
        function validatingJson(json: object, isBody: boolean, validator: ValidateFunction): IValidationResult {
          try {
            if (isBody) {
              const sorter = new RichContentSorter({ channel: Channels.RBM });
