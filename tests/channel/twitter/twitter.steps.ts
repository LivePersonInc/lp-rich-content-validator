...
const feature = loadFeature('tests/channel/twitter/twitter.feature');
...
defineFeature(feature, (test) => {
  test('Validation works for all eligible test-JSONs (High Lvl)', ({ given, when, then }) => {
    const eligibleTestJsonContainers = testJsonContainers.filter(
      (testJsonContainer) => testJsonContainer().channelCompatibility[Channels.TWITTER] !== undefined,
    );
    ...
    then('The validator either validates or returns errors for them as expected on the Twitter channel', () => {
      testExpectations.forEach((testExpectation) => {
        ...
        const {
          testJson: {
            channelCompatibility: { [Channels.TWITTER]: fbCompatibility },
          },
        } = testExpectation;
  ...
  test('Validation with exported Schemas works for all eligible test-JSONs (High Lvl)', ({ given, when, then }) => {
    ...
    given('I exported the Twitter Schemas from a Rich Content validator', () => {
      const validator = new RichContentValidator({ channel: Channels.TWITTER });
      const bodySchema = validator.exportSchema({ type: Types.BDY, channel: Channels.TWITTER });
      const metadataSchema = validator.exportSchema({ type: Types.MTD, channel: Channels.TWITTER });
      const quickrepliesSchema = validator.exportSchema({ type: Types.QUR, channel: Channels.TWITTER });
    ...
    when(
      ...
      () => {
        function validatingJson(json: object, isBody: boolean, validator: ValidateFunction): IValidationResult {
          try {
            if (isBody) {
              const sorter = new RichContentSorter({ channel: Channels.TWITTER });
