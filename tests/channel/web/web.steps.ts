...
const feature = loadFeature('tests/channel/web/web.feature');
...
defineFeature(feature, (test) => {
  test('Validation works for all eligible test-JSONs (High Lvl)', ({ given, when, then }) => {
    const eligibleTestJsonContainers = testJsonContainers.filter(
      (testJsonContainer) => testJsonContainer().channelCompatibility[Channels.WEB] !== undefined,
    );
    ...
    then('The validator either validates or returns errors for them as expected on the Web channel', () => {
      testExpectations.forEach((testExpectation) => {
        ...
        const {
          testJson: {
            channelCompatibility: { [Channels.WEB]: webCompatibility },
          },
        } = testExpectation;
  ...
  test('Validation with exported Schemas works for all eligible test-JSONs (High Lvl)', ({ given, when, then }) => {
    ...
    given('I exported the Web Schemas from a Rich Content validator', () => {
      const validator = new RichContentValidator({ channel: Channels.WEB });
      const bodySchema = validator.exportSchema({ type: Types.BDY, channel: Channels.WEB });
      const metadataSchema = validator.exportSchema({ type: Types.MTD, channel: Channels.WEB });
      const quickrepliesSchema = validator.exportSchema({ type: Types.QUR, channel: Channels.WEB });
